 ...
 var welcome_word = "你还在为背诵四六级词汇而苦恼吗？你还在为没过四六级而郁闷吗？我将告诉你,以后这一切都再不是问题，因为有它--词汇大战,一款英语单词记忆帮组小游戏,有了它,你将享受背诵单词的乐趣！";
 window.onload = init;
...
//对外接口函数
 function init()
 {
    ...
    draw_welcome();
     play_sound();
    //delate();
    setInterval(draw, SECONDSBETWEENFRAMES * 1000);   //屏幕刷新频率
...
//绘制单词
 function draw_words()
 {
    ...
    context2D.textBaseline = 'top';
 	context2D.fillText(word, WINDOWWITH/4+50, 225, 14);
 	context2D.fillText(meaning, WINDOWWITH/4+250, 225, 14);
 	context2D.restore();
 }
...
//绘制倒计时计数器
 ...
 function drawscore()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText("Score:" + score, 2*WINDOWWITH/3+200, 30, 14);
 	context2D.restore();
 ...
 function draw_stop_targe()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText("游戏暂停中......",WINDOWWITH/6,100,14);
 	context2D.restore();
 }
...
//绘制时间
 function showtime()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText("Time:" + seconds, 2*WINDOWWITH/3-50, 30, 14);
 	context2D.restore();
 }
...
function dorw_error_time()
{
	if (error_time>0) {
    	...
    	context2D.font = '  50px 微软雅黑';  
   	 	context2D.textAlign = 'left';  
   	 	context2D.textBaseline = 'top';
 		context2D.fillText("Error:" + error_time, 2*WINDOWWITH/3+200, 100, 14);
 		context2D.restore();
	}
	if (error_time==5) {
		isOver=true;
...
//字符随机生成
 ...
 function draw_end_word()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText("No more "+over_type+" to try ,you have died ! ", 2*WINDOWWITH/5-45, 300, 14);
 	context2D.fillText("Thanks to use word craft,your score is "+ score+".", 2*WINDOWWITH/5-45, 335, 14);
 	context2D.fillStyle = "#FF0000"; 
 	context2D.fillText("Click Me to again", 2*WINDOWWITH/5-45, 370, 14);
 	context2D.restore();
 ...
 function  draw_pass_word()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText("Congratulation , you pass "+level_word[level-1]+" Level ", 2*WINDOWWITH/5-45, 300, 14);
 	context2D.fillText("Thanks to use word craft,your score is "+ score+".", 2*WINDOWWITH/5-45, 335, 14);
 	context2D.fillStyle = "#009900"; 
 	context2D.fillText("Click Me to Next", 2*WINDOWWITH/5-45, 370, 14);
 	context2D.restore();
 ...
 function draw_game_level()
 {
    ...
    context2D.textAlign = 'left';  
    context2D.textBaseline = 'top';
 	context2D.fillText(level_word[level-1]+" Level", WINDOWWITH/4, 30, 30);
 	context2D.restore();
 ...
 function draw_welcome()
 {
    ...
    for (var i =0; i <= welcome_word.length; i++) {
    	context2D.fillText(welcome_word.charAt(i), word_width, word_height, 14);
    		word_width +=25;
